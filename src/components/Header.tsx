    ...
    <>
      ...
      <header>
        <nav>
          <ul>
            {urls.map((url, index) => {
              const isCurrentUrl = normalizePath(url.path) === currentPath;

              return (
                <li key={url.path}>
                  <div className={`anchor ${isCurrentUrl ? 'active' : ''}`}>
                    <Link className={`link-${index} ${isCurrentUrl ? 'active' : ''}`} to={url.path}>
                      {url.name}
                    </Link>
                  </div>
                </li>
